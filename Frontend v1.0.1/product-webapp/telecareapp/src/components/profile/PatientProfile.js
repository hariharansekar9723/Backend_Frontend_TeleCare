  ...
  const getPatientDetails = () => {
    UserService.getUserData("/api/v1/patient/" + localStorage.getItem("loginId"))
      .then(response => {
        ...
        //setProfileImg('data:image/jpeg;base64,' + hexToBase64(response.data.doctorImage));
        // const imageBlob = response.data.doctorImage.blob();
        // const reader = new FileReader();
        // reader.readAsDataURL(imageBlob);
        // const base64data = reader.result;
        // console.log('img url ---', base64data)
          ...
          <Card className="shadow p-3 mb-5 rounded" >
            <Row>
              <Col xs={12} md={3} >
                ...
                <Image className='m-auto w-100 h-100' src={patientData?.patientImage ? `data:image/jpeg;base64,${patientData.patientImage}` : "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png"} alt='user-pic' fluid rounded />
              ...
              {Object.keys(patientData).length === 0 ? <Spinner animation="border" role="status">
                ...
                <Card.Title className='px-5 d-flex flex-row-reverse'>
                  <Button variant="secondary" size="md" onClick={handleEditShow}>Edit &nbsp;<i className="bi bi-pencil"></i></Button>
                </Card.Title>
                <div className="d-md-flex justify-content-between px-5 py-3">
                  <div>
                    ...
                    <Card.Text>
                      <span className="fw-bold">DOB :</span> {moment(patientData.dob).format('Do MMMM YYYY')}
                    </Card.Text>
                    <Card.Text>
