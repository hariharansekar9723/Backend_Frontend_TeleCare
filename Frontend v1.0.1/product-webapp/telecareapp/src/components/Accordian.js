...
const Accordian = (props) => {
  ...
  const [slotAvilable, setSlotAvilable] = useState('true')
  ...
  function filterSlots() {
    console.log(date)
    let filteredSlots = []
    filteredSlots = props.slot.filter(x => x.doctorEmail == props.list.doctorEmailId).filter(x => x.slotDate == date).filter(x => x.slotAvailable == "true")
    setSlotAvilable(filteredSlots == [] ? 'true' : 'false') 
            ...
            <div className="accordion-body">
              <div className="row m-auto">
                ...
                {slot.length != 0 && <div className="col-12 mt-2">
                  <button className="btn btn-secondary w-100 bookBtn " onClick={() => bookAppointement()} disabled={currSlot == null} >Book Appoinment {currSlot == null} </button>
                </div>}
              </div>
            </div>
          </div>
